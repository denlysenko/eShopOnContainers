...
@Injectable()
export class OrderPaymentSucceededConsumer {
  constructor(
  ...
  @RabbitSubscribe({
    ...
    routingKey: OrderPaymentSucceededEvent.name,
  })
  async handle(event: OrderPaymentSucceededEvent): Promise<void | Nack> {
    this._logger.debug(
    ...
    const command = new SetPaidStatusCommand(event.orderId);
    ...
    this._logger.debug(
      `-- Sending command: ${SetPaidStatusCommand.name} - (${JSON.stringify(
        command
      )})`
    );
    ...
    try {
      ...
      this._logger.error(
        `-- Command: ${SetPaidStatusCommand.name} - (${JSON.stringify(
          command
