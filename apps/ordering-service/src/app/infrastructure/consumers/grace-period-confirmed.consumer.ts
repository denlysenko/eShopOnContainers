...
import {
  ...
  INBOX_REPOSITORY,
  Mediator,
  SetAwaitingValidationStatusCommand,
} from '../../application';

@Injectable()
export class GracePeriodConfirmedConsumer {
  constructor(
  ...
  @RabbitSubscribe({
    ...
    routingKey: GracePeriodConfirmedEvent.name,
  })
  async handle(event: GracePeriodConfirmedEvent): Promise<void | Nack> {
    this._logger.debug(
    ...
    const command = new SetAwaitingValidationStatusCommand(event.orderId);
    ...
    try {
      ...
      this._logger.error(
        `-- Command: ${
          SetAwaitingValidationStatusCommand.name
        } - (${JSON.stringify(command)}) was failed. Error: ${error}`
      );
