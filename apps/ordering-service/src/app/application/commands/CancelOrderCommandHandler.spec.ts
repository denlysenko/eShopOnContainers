...
import { CancelOrderCommandHandler } from './CancelOrderCommandHandler';
...
describe('CancelOrderCommandHandler', () => {
  ...
  beforeEach(() => {
    handler = new CancelOrderCommandHandler(ordersRepositoryMock, loggerMock);
    jest
  ...
  it('should get order from repository', async () => {
    await handler.handle(new CancelOrderCommand(orderNumber));
  ...
  it('should return Result.fail if order not found', async () => {
    ...
    expect(await handler.handle(new CancelOrderCommand(2))).toEqual(
  ...
  it('should return Result.fail if update repository failed', async () => {
    ...
    expect(await handler.handle(new CancelOrderCommand(orderNumber))).toEqual(
